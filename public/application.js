...
/***/ function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(10);
	var surveyStore = __webpack_require__(15);
	var thisSurveyStore = __webpack_require__(36);

	var onSurveyIndex = function onSurveyIndex(event) {
	  if (event && event.preventDefault) {
	    event.preventDefault();
	  }
	  api.surveyIndex().then(ui.successIndex).catch(ui.failureIndex);
	};

	var onSurveyShow = function onSurveyShow(event) {
	  var _this = this;

	  event.preventDefault();
	  var id = $(event.target).data("id");
	  api.surveyShow(id).then(function (response) {
	    var survey = response.survey;
	    thisSurveyStore.survey = survey;
	    //show user message if no questions in survey
	    survey.questions.length === 0 ? $('.empty-survey').text("There are currently no questions in this survey.") : $('.empty-survey').empty();

	    $('.submission-error').empty();

	    var yesAnswers = void 0;
	    var noAnswers = void 0;

	    for (var k = 0; k < survey.questions.length; k++) {
	      yesAnswers = 0;
	      noAnswers = 0;
	      for (var i = 0; i < survey.questions[k].answers.length; i++) {
	...
	var onSurveyCreate = function onSurveyCreate(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);

	  if (surveyStore.survey.length !== 0) {
	    var result = [];
	    for (var i = 0; i < surveyStore.survey.length; i++) {
	      if (data.survey.title === surveyStore.survey[i].title) {
	        result.push(surveyStore.survey[i].title);
	      }
	    }
	    if (result.length === 0) {
	      api.surveyCreate(data).then(function (response) {
	        store.surveyid = response.survey.id;
	        return store;
	      }).then(ui.successSurveyCreate).then(onSurveyIndex).catch(ui.failureSurveyCreate);
	    } else {
	...
	var onUpdateSurveyQuestion = function onUpdateSurveyQuestion(event) {
	  if (event && event.preventDefault) {
	    event.preventDefault();
	  }
	  ...
	  if (thisSurveyStore.survey.questions.length !== 0) {
	    ...
	    for (var i = 0; i < thisSurveyStore.survey.questions.length; i++) {
	    ...
	    if (result.length === 0) {
	      api.updateSurveyQuestion(data).then(onSurveyIndex).then(ui.successQuestionCreate).catch(ui.failureQuestionCreate);
	    } else {
	      result = [];
	...
	var onAnswer = function onAnswer(event) {
	  event.preventDefault();

	  var size = 0;
	  var specificSurvey = void 0;
	  for (var i = 0; i < surveyStore.survey.length; i++) {
	  ...
	  for (var _i = 0; _i < size; _i++) {
	    var id = specificSurvey.questions[_i].id;
	  ...
	  if (counter === problems.length) {
	    ...
	    api.updateAnswer(data, $(this).data("id")).then(ui.successAnswer).then(onSurveyIndex).catch(ui.failureAnswer);
	  } else {
	...
	var onDestroy = function onDestroy(event) {
	  ...
	  api.destroySurvey(id).then(onSurveyIndex).then(ui.successDestroy).catch(ui.failureDestroy);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var surveyShow = function surveyShow(id) {
	  return $.ajax({
	...
	var updateSurveyQuestion = function updateSurveyQuestion(data) {
	  return $.ajax({
	...
	var updateAnswer = function updateAnswer(data, id) {
	  return $.ajax({
	    url: config.apiOrigin + '/surveys/' + id,
	    method: 'PATCH',
	    headers: {
	...
	var destroySurvey = function destroySurvey(id) {
	  return $.ajax({
	    url: config.apiOrigin + '/surveys/' + id,
	    method: 'DELETE',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var surveyStore = __webpack_require__(15);
	var store = __webpack_require__(10);
	var surveyIndexHandlerbars = __webpack_require__(16);
	var api = __webpack_require__(13);
	...
	var successIndex = function successIndex(data) {
	  ...
	  var surveyHtml = surveyIndexHandlerbars({ surveys: data.surveys });
	...
	var successSurveyCreate = function successSurveyCreate(data) {
	  $('#survey-create')[0].reset();
	  $('.alerts').text('Created New Survey!');
	  $('.survey-message').empty();
	};

	var failureSurveyCreate = function failureSurveyCreate(error) {
	  $('.alerts').text('Survey name has already been created, choose new name');
	  events.onSurveyIndex();
	};

	var successQuestionCreate = function successQuestionCreate(data) {
	  $('.alerts').text('Added Question to Survey');
	  $('#question-create')[0].reset();
	};

	var failureQuestionCreate = function failureQuestionCreate(error) {};

	var successAnswer = function successAnswer(data) {
	  $('.alerts').text('Survey Answers Submitted');
	};

	var failureAnswer = function failureAnswer(error) {};

	var successDestroy = function successDestroy() {};

	var failureDestroy = function failureDestroy(error) {
	...
	module.exports = {
	  ...
	  successSurveyCreate: successSurveyCreate,
	  failureSurveyCreate: failureSurveyCreate,
	  successQuestionCreate: successQuestionCreate,
	  failureQuestionCreate: failureQuestionCreate,
	  successAnswer: successAnswer,
	  failureAnswer: failureAnswer,
	  successDestroy: successDestroy,
	  failureDestroy: failureDestroy
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "<div class=\"show-survey\" survey-id=\""
	    ...
	    + ">\n      <input type=\"button\" class=\"survey-show\" value=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.title : stack1), depth0))
	    + "\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1._id : stack1), depth0))
	    + "\">\n\n      <label class=\"survey-contents\">\n        "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.title : stack1), depth0))
	    + "\n        <div class=\"submission-error\"></div>\n        <h5 class=empty-survey></h3>\n        <h3 class=\"answer-length\"></h3>\n        <div class=\"add-question-form\">\n        <form class=\"style-form\" id=\"question-create\">\n          <fieldset>\n            <label>\n              <h3 class=\"question-fail\"></h3>\n              <input class=\"field-style fs1\" pattern=\".{1,}\" required title=\"1 characters minimum\" type=\"text\" name=\"survey[questions[problem]\" placeholder=\"Add Question To Survey\">\n              <input class=\"field-style\" type=\"text\" name=\"survey[id]\" placeholder=\"Survey ID\" value=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1._id : stack1), depth0))
	    + "\" style=\"display: none;\">\n            </label>\n            <input class=\"survey-button\" type=\"submit\" name=\"submit\" value=\"Add Question\">\n          </fieldset>\n        </form>\n        </div>\n        <input type=\"button\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1._id : stack1), depth0))
	    + "\" value=\"Delete Survey\" class=\"survey-destroy delete-style\">\n        <label class=\"questions-style\">\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},((stack1 = blockParams[0][0]) != null ? stack1.questions : stack1),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "        </label>\n        <input data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"answer-question\" type=\"submit\" name=\"submit\" value=\"Submit Answer\">\n    </label>\n    </fieldset>\n  </form>\n</div>\n";
	},"2":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

	  return "          <div class=\"survey-questions\">\n            <div class=\"survey-problem question-style\">"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.problem : stack1), depth0))
	    + "</div>\n          </div>\n        <form>\n          <fieldset>\n          <input class=\"survey-id\" value=\""
	    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1._id : stack1), depth0))
	    + "\" style=\"display: none;\">\n\n          <input class=\"survey-problem-id\" value=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1._id : stack1), depth0))
	    + "\" style=\"display: none;\">\n          <div class=\"box-radios\">\n          <div class=\"col-1\"><input type=\"radio\" class=\"col-1-radio\" name=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" value=true data-bind=\"checked:IsChecked, checkedValue: true\"/>\n            Yes\n            <div class=\"count-true\" name=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\"></div>\n          </div>\n          <div class=\"col-2\"><input type=\"radio\" class=\"col-2-radio\" name=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" value=false data-bind=\"checked:IsChecked, checkedValue: true\"/>\n            No\n            <div class=\"count-false\" name=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\"></div>\n          </div>\n          </div>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return "<div id=\"surveys-box\">\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.surveys : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</div>\n";
...
/***/ function(module, exports) {
	...
	module.exports = thisSurveyStore;
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "3f9bb7934e6b7b46d563db2e19aa72fe.png";
